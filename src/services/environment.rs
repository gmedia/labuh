...
impl EnvironmentService {
    ...
    pub async fn list(&self, stack_id: &str) -> Result<Vec<StackEnvVar>> {
        let vars = sqlx::query_as::<_, StackEnvVar>(
            "SELECT * FROM stack_env_vars WHERE stack_id = ? ORDER BY key"
        )
        .bind(stack_id)
        .fetch_all(&self.db)
        .await?;
    ...
    pub async fn get_env_map(&self, stack_id: &str) -> Result<std::collections::HashMap<String, String>> {
    ...
    pub async fn set(&self, stack_id: &str, key: &str, value: &str, is_secret: bool) -> Result<StackEnvVar> {
        let now = Utc::now().to_rfc3339();

        // Try to find existing
        let existing = sqlx::query_as::<_, StackEnvVar>(
            "SELECT * FROM stack_env_vars WHERE stack_id = ? AND key = ?"
        )
        .bind(stack_id)
        .bind(key)
        ...
        sqlx::query(
            "INSERT INTO stack_env_vars (id, stack_id, key, value, is_secret, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)"
        )
        .bind(&id)
        .bind(stack_id)
        .bind(key)
    ...
    pub async fn delete(&self, stack_id: &str, key: &str) -> Result<()> {
        let result = sqlx::query(
            "DELETE FROM stack_env_vars WHERE stack_id = ? AND key = ?"
        )
        .bind(stack_id)
        .bind(key)
        .execute(&self.db)
        ...
        if result.rows_affected() == 0 {
            return Err(AppError::NotFound(format!("Environment variable '{}' not found", key)));
        }
    ...
    pub async fn bulk_set(&self, stack_id: &str, vars: Vec<(String, String, bool)>) -> Result<Vec<StackEnvVar>> {
        let mut results = Vec::new();
        for (key, value, is_secret) in vars {
            let var = self.set(stack_id, &key, &value, is_secret).await?;
            results.push(var);
        }
        Ok(results)
    }
